  ...
  try {
    ...
    if (dom.matches) {
      dom.matches.innerHTML = `
        <div class="error-box">
          <strong>Maçlar yüklenemedi.</strong>
          <p>${escapeHtml(
            error.message
          )}</p>
        </div>
...
function renderLeagueMenu() {
  ...
  let html = `
    ...
    >
      ...
      <small>${state.fixtures.length}</small>
    </button>
  `;
...
async function openMatchAnalysis(
  ...
  // Cache içinde AI sonucu varsa direkt göster
  const cached =
  ...
  // Local cache varsa önce onu göster
  if (
    cached &&
    cached.apiData
  ) {
...
function buildAnalysis(
  ...
  return {
    ...
    expectedHome:
      round(expectedHome, 2),

    expectedAway:
      round(expectedAway, 2),
  ...
  dom.drawerContent.innerHTML = `
    ...
    ${
      ai
        ...
        `
        : `
          <section class="ai-analysis-panel ai-loading">
    ...
    <section class="local-analysis-panel">
      ...
      <div class="half-analysis local-half">
      ...
      <div class="analysis-disclaimer">
        İstatistiksel ve yapay zekâ destekli
        analizdir. Kesin sonuç garantisi vermez.
      </div>
